  ...
  <panel title="Printing Dashboard" icon="mdi-monitor-dashboard" card-class="blocks-main-panel" center-title>
    <div class="pa-1 h-100">
      <v-row align="stretch" justify="center"> 
        <v-col cols="12" lg="6" xl="5" class="pr-lg-6 mb-6 mb-lg-0 d-flex flex-column col-left-wrapper" style="gap: 24px;">
          <div class="dark-wrapper d-flex mb-4 flex-shrink-0" style="gap: 8px;">
            <v-btn class="tab-btn dark-wrapper flex-grow-1" :class="!showThumbnailView ? 'active-tab' : 'inactive-tab'" depressed @click="showThumbnailView = false">
            ...
            <v-btn class="tab-btn dark-wrapper flex-grow-1" :class="showThumbnailView ? 'active-tab' : 'inactive-tab'" depressed @click="showThumbnailView = true">
          ...
          <div class="dark-wrapper d-flex align-center pa-4 mt-4 overflow-hidden" style="min-height: clamp(100px, 12vh, 160px); flex: 0 0 auto;">
            <div class="d-flex align-center flex-grow-1 overflow-hidden">
              <v-progress-circular :rotate="-90" :size="64" :width="5" :value="displayProgress" color="var(--v-primary-base)" class="mr-5 flex-shrink-0">
              ...
              <div class="d-flex flex-column overflow-hidden flex-grow-1" style="min-width: 0; max-width: 220px;">
                <span class="white--text font-weight-bold text-uppercase primary--text" style="letter-spacing: 1px; font-size: 1rem; line-height: 1.3;">
                  {{ isStandby ? 'Standby' : (isPrinting ? 'Printing' : 'Paused') }}
                </span>
                <span class="white--text text-truncate" style="opacity: 0.7; font-size: 0.85rem;">
              ...
              <div v-if="!isStandby" class="d-flex align-center flex-shrink-0 justify-space-between px-2 text-center" style="gap: 20px;">
                <div class="d-flex flex-column align-center">
                  <span class="white--text" style="font-size: 0.65rem; opacity: 0.5; font-weight: 800; letter-spacing: 0.5px; white-space: nowrap;">ESTIMATE</span>
                  <span class="white--text font-weight-bold" style="font-size: 1rem; white-space: nowrap;">{{ printEstimate }}</span>
                </div>
                <div class="d-flex flex-column align-center">
                  <span class="white--text" style="font-size: 0.65rem; opacity: 0.5; font-weight: 800; letter-spacing: 0.5px; white-space: nowrap;">TOTAL</span>
                  <span class="white--text font-weight-bold" style="font-size: 1rem; white-space: nowrap;">{{ totalTime }}</span>
                </div>
                <div class="d-flex flex-column align-center">
                  <span class="white--text" style="font-size: 0.65rem; opacity: 0.5; font-weight: 800; letter-spacing: 0.5px; white-space: nowrap;">ETA</span>
            ...
            <div class="d-flex align-center flex-shrink-0 ml-4" style="gap: 14px;">
              <v-btn fab :color="isPrinting ? 'amber darken-2' : 'success darken-1'" depressed @click="dynamicStartAction">
                <img :src="'/img/icons/blocks_icons/' + dynamicStartIcon" width="24" height="24" style="filter: brightness(0) invert(1);" />
              </v-btn>
              <v-btn v-if="!isStandby" fab color="error" depressed @click="$socket.emit('printer.print.cancel')">
                <img src="/img/icons/blocks_icons/stopsvg.svg" width="24" height="24" style="filter: brightness(0) invert(1);" />
              </v-btn>
            </div>
          </div>
        </v-col>
          ...
          <div class="dark-wrapper pa-2 flex-shrink-0 mt-4">
            <v-row dense>
              <v-col cols="6" v-for="stat in mainStats" :key="stat.label" class="pa-1">
                <div class="dark-wrapper pa-3 d-flex align-center justify-space-between stat-container-fixed">
                  <span class="text-tiny white--text text-uppercase font-weight-bold">{{ stat.label }}</span>
                  <div class="white--text font-weight-bold ml-2 stat-value-text tabular-nums">
                    {{ stat.value }} <small class="white--text" style="font-size: 0.65rem">{{ stat.unit }}</small>
                  </div>
                </div>
              </v-col>
            </v-row>
        ...
        <v-col cols="12" md="6" lg="3" xl="3" class="pl-lg-6 d-flex flex-column">
          <div class="d-flex flex-column flex-grow-1" style="gap: 16px;">
            <div class="dark-wrapper pa-3 d-flex flex-column" style="gap: 8px;">
              <div v-for="temp in visibleTemperatureCards" :key="temp.n" class="dark-wrapper d-flex align-center px-3 py-3 flex-shrink-0">
                <img :src="'/img/icons/blocks_icons/' + temp.i" width="24" height="24" class="mr-3" style="filter: brightness(0) invert(1);" />
                <span class="white--text font-weight-bold text-truncate pr-2" style="flex: 1 1 auto; min-width: 0; font-size: 0.8rem;">{{ temp.n }}</span>
                <div class="d-flex align-center justify-end flex-shrink-0" style="gap: 10px;">
                  <div class="text-right">
                    ...
                    <div class="white--text font-weight-bold" style="font-size: 1rem; line-height: 1.3;">{{ temp.v }}°C</div>
                  </div>
            ...
            <div class="dark-wrapper d-flex flex-column align-center justify-center py-4 mb-0 flex-grow-1" style="min-height: 220px;">
...
/* Force stability on all screen sizes for the stat boxes */
.stat-container-fixed {
...
.standby-wrapper {
  ...
  background: var(--master-inner-bg) !important;
}
...
.tab-btn {
  ...
  border: 1px solid var(--master-inner-border) !important;
...
.fan-card-compact {
  ...
  margin-bottom: 8px;
  border-radius: var(--master-inner-radius);
}
