          ...
          <div class="d-flex align-center pa-4 mt-4 inner-rounded flex-shrink-0 overflow-hidden" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); min-height: 85px;">
            <div class="d-flex align-center flex-grow-1 overflow-hidden">
              ...
              <div v-if="!isStandby" class="d-flex align-center flex-grow-1 justify-space-around px-2 text-center overflow-hidden">
                ...
                <div class="d-flex flex-column"><span class="white--text" style="font-size: 0.6rem; opacity: 0.5; font-weight: bold;">SLICER EST.</span><span class="white--text font-weight-bold" style="font-size: 0.85rem;">{{ slicerEstimatedTime }}</span></div>
              ...
              <div v-for="(led, index) in displayLeds" :key="'led-'+index" class="fan-card-compact px-3 py-3">
                <div class="d-flex align-center justify-space-between mb-2 px-1">
                  <span class="white--text text-body-2 font-weight-bold d-flex align-center">
                    <img src="/img/icons/blocks_icons/LEDssvg.svg" width="18" height="18" class="mr-2" style="filter: brightness(0) invert(1);" /> {{ led.name }}
                  </span>
                ...
                <transition name="fade-slide">
                  <div v-if="led.enabled" class="d-flex align-center px-1" style="gap: 8px;">
                    <img src="/img/icons/blocks_icons/light_barsvg.svg" width="18" height="18" class="flex-shrink-0" style="filter: brightness(0) invert(1);" />
                    <v-slider v-model="led.brightness" :min="0" :max="100" :step="1" hide-details dense color="amber darken-1" track-color="rgba(255,255,255,0.12)" class="flex-grow-1 my-0" @change="onLedBrightnessChange(led)" />
                    <img src="/img/icons/blocks_icons/sun svg.svg" width="18" height="18" class="flex-shrink-0" style="filter: brightness(0) invert(1);" />
                    <span class="white--text text-caption font-weight-bold led-value-badge">{{ led.brightness }}%</span>
                  </div>
                </transition>
              </div>
            </div>
        ...
        <v-col cols="12" md="6" lg="3" xl="3" class="pl-lg-6">
          <div class="d-flex flex-column" style="gap: 16px;"> 
            <div v-for="temp in visibleTemperatureCards" :key="temp.n" class="temp-card d-flex align-center px-3 flex-shrink-0"> 
              <img :src="'/img/icons/blocks_icons/' + temp.i" width="22" height="22" class="mr-3 flex-shrink-0" style="filter: brightness(0) invert(1);" />
              ...
              <div class="d-flex align-center justify-end flex-shrink-0" style="gap: 8px;">
                ...
                <div v-if="temp.t != null" class="text-left pl-2" style="border-left: 1px solid rgba(255,255,255,0.3);">
                  ...
                  <div v-else class="white--text font-weight-bold" style="font-size: 0.9rem; line-height: 1.2;">{{ temp.t }}°C</div>
                </div>
              </div>
            </div>
...
interface LedItem {
  name: string; klipperName: string; brightness: number; enabled: boolean;
}
...
export default class OurPrintingPanel extends Mixins(BaseMixin, AfcMixin, WebcamMixin, MiscellaneousMixin) {
  ...
  mounted() { this.$root.$on('advancedModeChanged', (v: boolean) => { this.isAdvancedMode = v; }); this.fetchMoonrakerData(); }
  ...
  toggleFullscreen() { const el = this.$refs.mediaBox as HTMLElement; if (!document.fullscreenElement) el.requestFullscreen(); else document.exitFullscreen(); }
  ...
  get slicerEstimatedTime() {
    ...
    if (!est) {
      try {
        const metaFunc = this.$store.getters['files/getFileMetadata'];
        if (typeof metaFunc === 'function') est = metaFunc(this.activeFilename)?.estimated_time;
      } catch (_) {}
    }
    return est ? `${Math.floor(est / 3600)}h ${Math.floor((est % 3600) / 60)}m` : '--:--';
  }
  get totalTime() { 
    const p = this.printer?.display_status?.progress || 0;
    if (p > 0) {
...
.media-container:fullscreen, .media-container:-webkit-full-screen { aspect-ratio: auto !important; height: 100vh !important; }
...
.standby-content::-webkit-scrollbar { width: 6px; }
.standby-content::-webkit-scrollbar-track { background: rgba(221, 221, 221, 0.02); border-radius: 4px; }
.standby-content::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.2); border-radius: 4px; }
.standby-content::-webkit-scrollbar-thumb:hover { background: rgba(255, 255, 255, 0.4); }

@media (max-width: 599px) { ::v-deep .fan-card-compact .v-slider { display: none !important; } }
