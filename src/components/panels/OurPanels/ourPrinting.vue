      ...
      <v-row align="stretch" justify="center"> <v-col cols="12" lg="6" xl="5"
          ...
          <div class="media-container flex-grow-1 shadow-xl relative overflow-hidden" ref="mediaBox">
            <div v-if="!showThumbnailView" class="h-100 w-100 absolute-fill-wrapper d-flex align-center justify-center">
              <webcam-wrapper v-if="isLive" :webcam="currentCam" page="dashboard"
                class="h-100 w-100 inner-rounded webcam-hero" />
            </div>
            <div v-else class="h-100 w-100 absolute-fill-wrapper d-flex flex-column">
              <div v-if="!isStandby" class="thumbnail-wrapper h-100 w-100 relative inner-rounded flex-grow-1">
                <v-img v-if="thumbnailUrl" :src="thumbnailUrl" class="responsive-thumbnail inner-rounded h-100 w-100"
                  contain />
                <div v-else
                  class="no-thumb-bg d-flex flex-column align-center justify-center inner-rounded h-100 w-100 absolute-fill-wrapper">
                  <img src="/img/icons/blocks_icons/printsvg.svg" width="64" height="64"
              ...
              <div v-else class="standby-wrapper d-flex flex-column h-100 w-100 inner-rounded absolute-fill-wrapper">
                <div class="standby-header d-flex align-center justify-space-between pa-4 flex-shrink-0">
                  <div class="d-flex align-center">
                    <img src="/img/icons/blocks_icons/infosvg.svg" width="20" height="20" class="mr-3"
                      style="filter: brightness(0) invert(1);" />
                    <span class="white--text font-weight-regular text-h6">Standby</span>
                  </div>
                  <img src="/img/icons/blocks_icons/downsvg.svg" width="16" height="16"
                    style="filter: brightness(0) invert(1);" />
                </div>
                <div class="standby-mini-tabs d-flex flex-shrink-0 mx-4">
                  ...
                  <div class="mini-tab" style="margin-left: 60px;" :class="standbyTab === 1 ? 'active-mini-tab' : ''"
                    @click="standbyTab = 1">
                ...
                <div class="standby-content flex-grow-1 px-2 py-2">
                  <template v-if="standbyTab === 0">
                    <div v-for="(file, index) in recentFiles" :key="index"
                      class="history-item d-flex align-center pa-3 mx-1">
                      <div class="mr-4 d-flex align-center justify-center flex-shrink-0"
                        style="width: 32px; height: 32px; background: rgba(0,0,0,0.2); border-radius: 4px;">
                        <img v-if="file.thumbUrl" :src="file.thumbUrl"
                          style="max-width: 100%; max-height: 100%; object-fit: contain; border-radius: 4px;" />
                        <img v-else src="/img/icons/blocks_icons/file_iconsvg.svg" width="20" height="20"
                          style="filter: brightness(0) invert(1); opacity: 0.7;" />
                      </div>
                      <div class="flex-grow-1 overflow-hidden pr-3">
                        <div class="white--text text-body-2 text-truncate">{{ file.name }}</div>
                        <div class="white--text text-caption text-truncate" style="opacity: 0.7;">Filament: {{
                          file.filament }} | Time: {{ file.time }}</div>
                      </div>
                      <img :src="'/img/icons/blocks_icons/' + file.statusIcon" width="20" height="20" />
                    </div>
                  </template>
                  <template v-else-if="standbyTab === 1">
                    <div v-for="(job, index) in queuedJobs" :key="'job-' + index"
                      class="history-item d-flex align-center pa-3 mx-1">
                      <img src="/img/icons/blocks_icons/timesvg.svg" width="24" height="24" class="mr-4"
                        style="filter: brightness(0) invert(1); opacity: 0.7;" />
                      <div class="flex-grow-1 overflow-hidden pr-3">
            ...
            <div class="media-controls-overlay d-flex justify-start align-end pa-3"
              v-if="!isStandby || !showThumbnailView">
        ...
        <v-col cols="12" md="6" lg="3" xl="3" class="pl-lg-6 d-flex flex-column">
          <div class="d-flex flex-column flex-grow-1" style="gap: 16px;">
            ...
            <div class="dark-wrapper d-flex flex-column align-center justify-center py-4 mb-0 flex-grow-1"
              ...
              <span class="white--text font-weight-regular text-body-1 mb-3" style="opacity: 0.85;">Clog/Tangle
                Detection</span>
