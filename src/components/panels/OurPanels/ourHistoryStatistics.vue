<template>
  <v-card class="unified-stats-panel pa-6 mb-6">
    <div class="d-flex align-center mb-6">
      <img 
        ...
        alt="Statistics Icon"
      />
      <span class="text-h6 font-weight-bold white--text">{{ $t('History.Statistics') }}</span>
      <v-spacer></v-spacer>
      <v-btn icon @click="collapsed = !collapsed" color="white">
      ...
      <div v-show="!collapsed">
        <v-row dense class="mb-6">
          <v-col v-for="total in totals" :key="total.title" cols="6" sm="4" md="auto" class="flex-grow-1">
            <div class="stat-mini-box pa-4 d-flex flex-column justify-center align-center text-center">
              <span class="text-caption grey--text text--lighten-1 mb-1 text-uppercase font-weight-bold" style="letter-spacing: 0.5px; font-size: 0.65rem !important;">
                {{ total.title }}
              </span>
        ...
        <v-row dense>
          <v-col cols="12" md="6">
            <div class="internal-chart-card pa-4 d-flex flex-column">
              <div class="d-flex justify-space-between align-center mb-4">
                <v-btn-toggle v-model="toggleValue" small mandatory class="custom-toggle-group">
                  <v-btn v-for="option in toggleValueOptions" :key="option.value" x-small :value="option.value">
                    {{ option.text }}
                  </v-btn>
                </v-btn-toggle>
                
                <div class="d-flex align-center">
                  <v-btn-toggle v-model="togglePrintStatus" small mandatory class="mr-2 custom-toggle-group">
                    <v-btn x-small value="chart">{{ $t('History.Chart') }}</v-btn>
                    <v-btn x-small value="table">{{ $t('History.Table') }}</v-btn>
                  </v-btn-toggle>

                  <v-btn icon x-small color="primary" :loading="loadings.includes('historyLoadAll')" @click="refreshHistory">
                    <v-icon x-small>{{ mdiDatabaseArrowDownOutline }}</v-icon>
                  </v-btn>
                </div>
              </div>
              
              <div class="flex-grow-1 d-flex align-center justify-center" style="min-height: 250px;">
          ...
          <v-col cols="12" md="6">
            <div class="internal-chart-card pa-4 d-flex flex-column">
              <div class="d-flex justify-center mb-4">
                <v-btn-toggle v-model="toggleChart" small mandatory class="custom-toggle-group">
                  <v-btn x-small value="filament_usage">{{ $t('History.FilamentUsage') }}</v-btn>
                  <v-btn x-small value="printtime_avg">{{ $t('History.PrinttimeAvg') }}</v-btn>
                </v-btn-toggle>
              </div>
...
import { HistoryStatsValueNames, ServerHistoryStateJob, ServerHistoryStateJobAuxiliaryTotal } from '@/store/server/history/types'
...
export default class OurHistoryStatistics extends Mixins(BaseMixin, HistoryMixin) {
    ...
    get existsSelectedJobs() { return this.selectedJobs.length > 0 }
    get totalPrintTime() { return this.$store.state.server.history.job_totals?.total_print_time ?? 0 }
    get selectedPrintTime() {
        let printtime = 0
        this.selectedJobs.forEach((job: ServerHistoryStateJob) => { printtime += job.print_duration })
        return printtime
    }
    get longestPrintTime() { return this.$store.state.server.history.job_totals?.longest_print ?? 0 }
    get selectedLongestPrintTime() {
        let printtime = 0
        this.selectedJobs.forEach((job: ServerHistoryStateJob) => { if (job.print_duration > printtime) printtime = job.print_duration })
        return printtime
    }
    get avgPrintTime() {
    ...
    get totalFilamentUsed() { return this.$store.state.server.history.job_totals?.total_filament_used ?? 0 }
    ...
    get selectedFilamentUsed() {
        let filamentUsed = 0
        this.selectedJobs.forEach((job: ServerHistoryStateJob) => { filamentUsed += job.filament_used })
        return filamentUsed
    }
    ...
    get totalJobsCount() { return this.$store.state.server.history.job_totals?.total_jobs ?? 0 }
    get toggleChart() { return this.$store.state.gui.view.history.toggleChartCol3 }
    set toggleChart(newVal) { this.$store.dispatch('gui/saveSetting', { name: 'view.history.toggleChartCol3', value: newVal }) }
    get togglePrintStatus() { return this.$store.state.gui.view.history.toggleChartCol2 }
    set togglePrintStatus(newVal) { this.$store.dispatch('gui/saveSetting', { name: 'view.history.toggleChartCol2', value: newVal }) }
    get allLoaded() { return this.$store.state.server.history.all_loaded ?? false }
    ...
    get totals() { return this.existsSelectedJobs ? this.selectedTotals : this.genericTotals }
...
.custom-toggle-group .v-btn {
    border: none !important;
    background: transparent !important;
    color: rgba(255, 255, 255, 0.6) !important;
    font-weight: bold;
}
.custom-toggle-group .v-btn--active {
